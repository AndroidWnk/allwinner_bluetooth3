package com.etrans.bluetooth.db;
...
public class Database {
	...
	private static String mDbPath = "/data/data/com.etrans.bluetooth/BtPhone.db";
	...
	public static List<ContactInfos> queryAllContacts(SQLiteDatabase db){  //查询所有联系人
		...
		Cursor cursor = db.query(PhoneBookTable, new String[] { "phonename",
				"phonenumber" }, null, null, null, null, null);
		List<ContactInfos> contacts = new ArrayList<ContactInfos>();
		ContactInfos contactInfo = null;
		while(cursor.moveToNext()){
			contactInfo = new ContactInfos();
			contactInfo.num = cursor.getString(cursor.getColumnIndex("phonenumber"));
			contactInfo.name = cursor.getString(cursor.getColumnIndex("phonename"));
			contacts.add(contactInfo);
		}
		cursor.close();
		return contacts;
	...
	// 向通话记录表中插入一条数据
		...
		public static List<CallLogInfo> queryCallLog(SQLiteDatabase db, int CallType) {
			...
			while (cursor.moveToNext()) {
				...
				callloginfo.name = Phonename;
				callloginfo.number = Phonenum;
				callloginfo.type = CallType;
