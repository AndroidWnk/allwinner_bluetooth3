...
public class GocDatabase {
    private static final String DB_PATH = "/data/data/com.etrans.bluetooth/BtPhone.db";
    ...
    public List<ContactInfos> getAllPhonebooks() {
        List<ContactInfos> list = new ArrayList<ContactInfos>();
        ContactInfos contactInfo;
        Cursor cursor = queryPhonebook();
        for(cursor.moveToFirst();!cursor.isAfterLast();cursor.moveToNext()) {
            contactInfo = new ContactInfos();
            contactInfo.name = cursor.getString(cursor.getColumnIndex(COL_NAME));
            contactInfo.num = cursor.getString(cursor.getColumnIndex(COL_NUMBER));
            list.add(contactInfo);
        }
